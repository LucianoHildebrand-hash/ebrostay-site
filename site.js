const translations = {
  es: {
    ...
    "availability.kicker": "Disponibilidad",
    "availability.title": "Filtra propiedades por fechas.",
    "availability.copy": "Selecciona entrada, salida y número de huéspedes para ver qué propiedades de ejemplo están disponibles. Las fechas se pueden conectar más adelante a calendarios reales.",
    ...
    "availability.matches": "{count} propiedades disponibles para esas fechas.",
    "availability.oneMatch": "1 propiedad disponible para esas fechas.",
    "availability.none": "No hay propiedades disponibles para esas fechas y capacidad.",
    ...
    "availability.from": "Desde {price}/noche",
  ...
  en: {
    ...
    "availability.kicker": "Availability",
    "availability.title": "Filter properties by date.",
    "availability.copy": "Choose check-in, check-out, and guests to see which sample properties are available. The dates can later connect to real calendars.",
    ...
    "availability.matches": "{count} properties available for those dates.",
    "availability.oneMatch": "1 property available for those dates.",
    "availability.none": "No properties are available for those dates and capacity.",
    ...
    "availability.from": "From {price}/night",
...
const properties = [
  {
    id: "pilar",
    nameKey: "properties.pilar.name",
    areaKey: "properties.pilar.area",
    copyKey: "properties.pilar.copy",
    guests: 4,
    price: "92 EUR",
    unavailable: [
  ...
  {
    id: "actur",
    nameKey: "properties.actur.name",
    areaKey: "properties.actur.area",
    copyKey: "properties.actur.copy",
    guests: 2,
    price: "76 EUR",
    unavailable: [
  ...
  {
    id: "romareda",
    nameKey: "properties.romareda.name",
    areaKey: "properties.romareda.area",
    copyKey: "properties.romareda.copy",
    guests: 6,
    price: "128 EUR",
    unavailable: [
...
function isAvailable(property, filter) {
  ...
  if (property.guests < filter.guests) {
    return false;
  }

  return !property.unavailable.some(([start, end]) => {
    return rangesOverlap(filter.checkIn, filter.checkOut, dateValue(start), dateValue(end));
  });
}

function renderProperties() {
  if (!propertyGrid || !availabilityStatus) {
    return;
  }

  const rendered = properties.map((property) => {
    const available = isAvailable(property, activeFilter);
    return { ...property, available };
  });

  const visible = activeFilter ? rendered.filter((property) => property.available) : rendered;
  const count = visible.length;

  if (availabilityMessageOverride) {
    availabilityStatus.textContent = availabilityMessageOverride;
  } else if (!activeFilter) {
    availabilityStatus.textContent = t("availability.all");
  } else if (count === 0) {
    availabilityStatus.textContent = t("availability.none");
  } else if (count === 1) {
    availabilityStatus.textContent = t("availability.oneMatch");
  } else {
    availabilityStatus.textContent = interpolate("availability.matches", { count });
  }

  propertyGrid.innerHTML = visible.map((property) => {
    const pillClass = property.available ? "is-open" : "is-blocked";
    const pillText = property.available ? t("availability.open") : t("availability.blocked");

    return `
      <article class="property-card${property.available ? "" : " is-unavailable"}">
        <span class="availability-pill ${pillClass}">${pillText}</span>
        <div>
...
if (availabilityFilter) {
  availabilityFilter.addEventListener("submit", (event) => {
    ...
    const formData = new FormData(availabilityFilter);
    const checkIn = dateValue(formData.get("checkIn"));
    const checkOut = dateValue(formData.get("checkOut"));
    const guests = Number(formData.get("guestCount")) || 1;
    ...
    activeFilter = { checkIn, checkOut, guests };
    renderProperties();
  });
}
